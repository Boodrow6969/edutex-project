...
export default function StoryboardPage() {
  ...
  const { pageId, isLoading, error, notFound } = usePageByType(courseId, 'STORYBOARD');
      ...
      <div className="min-h-full">
        <div className="bg-white border-b border-gray-200 px-6 py-3">
          <nav className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <span>/</span>
            <span className="text-gray-900">Storyboard</span>
          </nav>
        </div>
        <div className="p-8 text-center">
          <p className="text-gray-600">Storyboard hasn&apos;t been created yet.</p>
          <Link href={`/workspace/${workspaceId}/course/${courseId}`} className="inline-block mt-4 text-blue-600 hover:text-blue-700">
            ← Back to Course Dashboard
          </Link>
        </div>
  ...
  return (
    <div className="min-h-full flex flex-col">
      ...
      <div className="flex-1 overflow-hidden">
        <StoryboardEditor pageId={pageId} courseId={courseId} workspaceId={workspaceId} />
      </div>
    </div>
  );
}
